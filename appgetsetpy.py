...
if data is not None:
    ...
    model_path = "housepricingmodel.joblib"  # Path to your joblib file
    if not os.path.exists(model_path):
        st.error("❌ Model not found! Please ensure 'model.joblib' is in the same directory.")
        st.stop()
    ...
    with tab2:
        ...
        fig1.add_trace(go.Scatter(x=data['area'], y=p(data['area']),
                                   mode='lines', name='Trend Line', line=dict(color='red', width=2)))
